  ...
  const STACK_COLOR_DEFAULT = "border-dark-200/70 bg-white/80 text-dark-500 dark:border-dark-600/50 dark:bg-dark-700/60 dark:text-dark-200";
  ...
  const CATEGORY_COLOR_DEFAULT = "border-white/15 bg-white/10 text-dark-600 dark:text-dark-100";
  ...
  const CardBadges = ({ category, date }) => {
    ...
    return (
      <div className="absolute left-3 top-3 flex flex-wrap items-center gap-1.5">
        <span className={`rounded-full border px-2.5 py-0.5 text-[0.6rem] font-semibold uppercase tracking-[0.18em] backdrop-blur-sm ${colorClasses}`}>
          {category}
        </span>
        {date && (
  ...
  const ImageHoverOverlay = ({ hasLiveUrl }) => {
    if (!hasLiveUrl) return null;

    return (
      <div
        aria-hidden="true"
        className="absolute inset-0 flex items-center justify-center bg-dark-900/55 opacity-0 backdrop-blur-[1px] transition-opacity duration-300 group-hover:opacity-100"
      >
        <span className="flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold text-white backdrop-blur">
  ...
  const StackList = ({ stack }) => (
    <ul className="flex flex-wrap gap-1.5" aria-label="Tecnologías utilizadas">
      {stack.map((tech) => (
        <li
          key={tech}
          className={`rounded-full border px-2 py-0.5 text-[0.62rem] font-medium transition-colors duration-200 ${STACK_COLORS[tech] ?? STACK_COLOR_DEFAULT}`}
        >
          {tech}
        </li>
      ))}
    ...
    <div className="mt-4 flex items-center gap-2">
      ...
      {href ? (
        <a
          ...
          rel="noopener noreferrer"
          onClick={(e) => e.stopPropagation()}
          aria-label="Ver demo en vivo del proyecto"
      ...
      <article className="group relative flex h-full flex-col overflow-hidden rounded-[24px] border border-dark-200/60 bg-white shadow-[0_20px_60px_-30px_rgba(0,0,0,0.12)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-primary-400/30 hover:shadow-[0_28px_70px_-25px_rgba(0,0,0,0.18)] dark:border-white/[0.08] dark:bg-dark-900/80 dark:shadow-[0_20px_60px_-30px_rgba(0,0,0,0.6)] dark:hover:border-primary-400/20 dark:hover:shadow-[0_28px_70px_-25px_rgba(0,0,0,0.75),0_0_0_1px_rgba(99,179,237,0.12)]">
        ...
        <div className="relative mx-4 mt-4 aspect-[16/9] overflow-hidden rounded-xl border border-dark-200/40 bg-dark-100/50 dark:border-white/[0.08] dark:bg-dark-800">
          <img
            ...
            decoding="async"
          />
          <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-gradient-to-t from-dark-900/60 via-transparent to-transparent" />
          <ImageHoverOverlay hasLiveUrl={Boolean(href)} />
        ...
        <div className="flex flex-1 flex-col px-5 pb-5 pt-4">
          ...
          <div aria-hidden="true" className="my-4 h-px w-full bg-dark-200/60 dark:bg-white/[0.06]" />
          <StackList stack={stack} />
          <CardActions github={github} href={href} />
        </div>
      </article>
    );
  };